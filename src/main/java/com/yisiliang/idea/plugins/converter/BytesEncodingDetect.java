...
/**
 * <Detect encoding .>
 * Copyright (C) <2009>  <a href="Fluck,ACC">http://androidos.cc/dev</a>
 * <p>
...
public class BytesEncodingDetect {
     ...
     */
    public Charset detectEncoding(byte[] rawtext) {
        int index, maxScore = 0;
        int encoding_guess = EncodingConstants.OTHER;
        int[] scores = new int[EncodingConstants.TOTAL_TYPES];
        // Assign Scores
        scores[EncodingConstants.GB18030] = gb18030_probability(rawtext);
        scores[EncodingConstants.BIG5] = big5_probability(rawtext);
        scores[EncodingConstants.UTF8] = utf8_probability(rawtext);
        scores[EncodingConstants.UTF32BE] = utf32be_probability(rawtext);
        if (scores[EncodingConstants.UTF32BE] == MAX_SCORE) {
            return EncodingConstants.SUPPORT_CHARSET_ARRAY[EncodingConstants.UTF32BE];
        }
        scores[EncodingConstants.UTF32LE] = utf32le_probability(rawtext);
        if (scores[EncodingConstants.UTF32LE] == MAX_SCORE) {
            return EncodingConstants.SUPPORT_CHARSET_ARRAY[EncodingConstants.UTF32LE];
        }
        scores[EncodingConstants.UTF16BE] = utf16be_probability(rawtext);
        scores[EncodingConstants.UTF16LE] = utf16le_probability(rawtext);
        scores[EncodingConstants.EUC_KR] = euc_kr_probability(rawtext);
        scores[EncodingConstants.CP949] = cp949_probability(rawtext);
        scores[EncodingConstants.ASCII] = ascii_probability(rawtext);
        scores[EncodingConstants.EUC_JP] = euc_jp_probability(rawtext);
        scores[EncodingConstants.OTHER] = 0;
        // Tabulate Scores
        for (index = 0; index < EncodingConstants.TOTAL_TYPES; index++) {
        ...
        if (maxScore <= 50) {
            encoding_guess = EncodingConstants.OTHER;
        }
        return EncodingConstants.SUPPORT_CHARSET_ARRAY[encoding_guess];
    }
    ...
    private int utf8_probability(byte[] rawtext) {
        ...
        int score = (int) (MAX_SCORE * ((float) goodBytes / (float) (rawTextLen - asciiBytes)));
        if (score > 98) {
            return score;
     ...
     */
    private int utf16be_probability(byte[] rawtext) {
    ...
    private int utf16le_probability(byte[] rawtext) {
        if (rawtext.length > 1 && ((byte) 0xFF == rawtext[0] && (byte) 0xFE == rawtext[1])) { // Little-endian
            return MAX_SCORE;
        }
        return 0;
    }

    private int utf32be_probability(byte[] rawtext) {
        if (rawtext.length > 3 && (
                (byte) 0x00 == rawtext[0]
                        && (byte) 0x00 == rawtext[1]
                        && (byte) 0xFE == rawtext[2]
    ...
    private int utf32le_probability(byte[] rawtext) {
        if (rawtext.length > 3 && (
                (byte) 0xFF == rawtext[0]
                        && (byte) 0xFE == rawtext[1]
                        && (byte) 0x00 == rawtext[2]
     ...
     */
    private int ascii_probability(byte[] rawtext) {
        int score = 75;
        int i, rawTextLen;
     ...
     */
    private int euc_kr_probability(byte[] rawtext) {
     ...
     */
    private int cp949_probability(byte[] rawtext) {
        int dbChars = 1, krChars = 1;
        long krFreq = 0, totalFreq = 1;
        int row, column;
        // Stage 1: Check to see if characters fit into acceptable ranges
    ...
    private void initializeGBFrequencies() {
        for (int[] array : GBFreq) {
            Arrays.fill(array, 0);
        }
        GBFreq[20][35] = 599;
        GBFreq[49][26] = 598;
    ...
    private void initializeGBKFrequencies() {
        for (int[] array : GBKFreq) {
            Arrays.fill(array, 0);
        }
        GBKFreq[52][132] = 600;
        GBKFreq[73][135] = 599;
    ...
    private void initializeBig5Frequencies() {
        for (int[] array : Big5Freq) {
            Arrays.fill(array, 0);
        }
        Big5Freq[9][89] = 600;
        Big5Freq[11][15] = 599;
    ...
    private void initializeJPFrequencies() {
        for (int[] array : JPFreq) {
            Arrays.fill(array, 0);
        }
        JPFreq[3][74] = 600;
        JPFreq[3][45] = 599;
    ...
    private void initializeKRFrequencies() {
        for (int[] array : KRFreq) {
            Arrays.fill(array, 0);
        }
    ...
    private void initializeFrequencies() {
        initializeGBFrequencies();
        initializeGBKFrequencies();
        initializeBig5Frequencies();
        initializeJPFrequencies();
        initializeKRFrequencies();
    }
}
